...
3. **Aggregation is frontend-only, in `buildTreeData`.** The workspace partitions and project tree are already on the client; a server aggregation endpoint would invent coupling for <100 workspaces. The aggregator is a pure tree-walk over `childrenByParent` + a union of `workspacesByProject` entries.
