...
| Create Workspace from PR | Create a workspace from a pull request |
| Open in IDE | Open the workspace in your configured editor |
| Copy Path | Copy the workspace path to clipboard |
| Copy Raw Logs | Copy the current session's raw logs |
| Toggle Dev Server | Start or stop the dev server |
| Projects Guide | Open the onboarding guide |
| Project Settings | Open the current project's settings |
