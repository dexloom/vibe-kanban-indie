...
description: "Understanding the Workspaces view layout, the global sidebar, and navigation"
---

<Frame>
<img style={{maxHeight: "400px"}} src="/images/workspaces-interface-overview.png" alt="Workspaces interface showing the global sidebar with the conversation, context, and details panels" />
</Frame>

The Workspaces view is where you drive a single workspace. Navigation — projects, boards, and the list of workspaces — lives in the **global sidebar** on the far left of the app (see [Projects](/board/projects)); the view itself is built from three panels.
...
| Icon | Action | Description |
|:----:|--------|-------------|
| <Icon icon="robot" /> | Spawn Orchestrator | Start the singleton orchestrator agent |
| <Icon icon="list" /> | Command Bar | Open the command bar |
| <Icon icon="circle-question" /> | Projects Guide | Open the onboarding guide |
...
Projects, boards, and workspaces all live in the **global sidebar** on the far left of the app, in the same place from every screen. It is a project-scoped tree:

- **Projects** at the top, with a **+** to create one. A project can hold **child boards** (see [Projects](/board/projects)).
- Under each project: a **Tasks** section (the project's issue cards) and, on root projects only, a **Workspaces** section that aggregates every workspace linked anywhere in that project's subtree.
- **Workspaces** appear as leaves, grouped into **Active / Running / Idle / Archived** buckets. Click a leaf to open that workspace.
- A project with an **orchestrator prompt** shows a prompt node (see [Orchestrator](/cockpit/orchestrator)).

<Frame>
<img style={{maxHeight: "350px"}} src="/images/workspaces-sidebar-accordion.png" alt="Sidebar tree grouping workspaces by Needs Attention, Idle, and Running" />
</Frame>

For a flat, cross-project list of workspaces with search and filtering, open the **Workspaces dashboard** (`/workspaces`).

### Workspace status indicators

Each workspace leaf shows its current state:
...
Start a new workspace from the **Workspaces dashboard**, or from an issue's **Workspaces** section (its **+** links a workspace to that issue). The interface switches to create mode:
...
<Note>
The integrated terminal runs commands directly in the workspace's own environment.
</Note>
...
<Note>
Workspace notes save automatically and are private to each workspace.
</Note>
