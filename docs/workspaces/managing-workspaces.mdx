...
<Note>
**Workspace Actions** are available from the command bar. These actions require an active workspace - they won't appear on the create workspace page.
</Note>

## Accessing Workspace Actions

Workspace actions (rename, duplicate, pin, archive, delete) live in the command bar; the navbar also has a quick **Archive** button.
...
By default, worktrees are stored under your home directory at `~/.vibe-kanban/worktrees` on all platforms. This persistent location keeps worktrees safe from system temp-directory cleanup.

| Platform | Default Location |
|----------|-----------------|
| **macOS** | `~/.vibe-kanban/worktrees` |
| **Linux** | `~/.vibe-kanban/worktrees` |
| **Windows** | `%USERPROFILE%\.vibe-kanban\worktrees` |
...
<Tab title="Windows">
```powershell
Get-ChildItem "$env:USERPROFILE\.vibe-kanban\worktrees" -Recurse | Measure-Object -Property Length -Sum
```
</Tab>
</Tabs>
