...
`Host` header is not a loopback authority (`localhost`, `127.0.0.0/8`, `[::1]`,
any port) — the defence against a public website resolving its own hostname to
`127.0.0.1` and driving your unauthenticated board from your browser. Reaching the
app by a LAN IP, tunnel, or VPN name needs that name in the `VK_ALLOWED_HOSTS`
environment variable *as well as* an entry here.
