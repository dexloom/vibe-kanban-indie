...
The General tab contains application-wide settings for appearance, default agent, editor configuration, git preferences, network access, and notifications.
...
<Tip>
Use a prefix that matches your branching conventions (e.g., `feature`, `fix`, or your initials).
</Tip>
...
By default the backend only accepts browser requests from `localhost`, so accessing
the app from another machine on your LAN (or through a tunnel or VPN) fails with a
silent 403 — the board spins forever and project creation errors out. The **Allowed
Origins** field lets you add those extra origins from the UI, **without restarting
the server**.

- **Allowed Origins** - A comma-separated list of extra `http(s)` URLs the backend
  should accept (e.g. `http://192.168.1.50:3001, https://my-tunnel.example.com`).
  Each entry is validated as a URL on save.
- **Loopback is always allowed.** `localhost` and same-origin requests pass
  regardless of this list, so you cannot lock yourself out of Settings.

<Note>
If the list is empty, the backend falls back to the `VK_ALLOWED_ORIGINS`
environment variable (set by the dev script to your frontend port). Adding origins
here overrides that seed; clearing the list restores the env-based behaviour.
</Note>

<Warning>
**Allowed Origins is only half of it.** The backend also rejects any request whose
`Host` header is an unrecognised **hostname** — the defence against a public
website resolving its own hostname to your machine and driving your
unauthenticated board from a visitor's browser. `localhost` and any IP address
literal are always accepted, so reaching the app by a LAN IP needs nothing;
reaching it by a tunnel, VPN, or proxy **name** needs that name in the
`VK_ALLOWED_HOSTS` environment variable *as well as* an entry here.

`VK_ALLOWED_HOSTS` is intentionally **not** editable from this page: a rejected
`Host` returns 403 for every route, Settings included, so a UI knob could never be
used to recover. Set it in the environment and restart. See
